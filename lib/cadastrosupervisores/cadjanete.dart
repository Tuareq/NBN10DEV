...
class CadJanete extends StatefulWidget {
  const CadJanete({Key? key}) : super(key: key);

  @override
  State<CadJanete> createState() => _CadJaneteState();
}

class _CadJaneteState extends State<CadJanete> {
  ...
  Future<void> loadSavedData() async {
    ...
    setState(() {
      _nomeController.text = prefs.getString('user3_nome') ?? '';
      _emailController.text = prefs.getString('user3_email') ?? '';
      _matriculaController.text = prefs.getString('user3_matricula') ?? '';
      _phoneController.text = prefs.getString('user3_telefone') ?? '';
      _equipeController.text = prefs.getString('user3_equipe') ?? '';
    });
  }
  ...
  Future<void> saveData() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('user3_nome', _nomeController.text);
    await prefs.setString('user3_email', _emailController.text);
    await prefs.setString('user3_matricula', _matriculaController.text);
    await prefs.setString('user3_telefone', _phoneController.text);
    await prefs.setString('user3_equipe', _equipeController.text);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Cadastro Supervisor'),
      ),
