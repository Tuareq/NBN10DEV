...
class CadNbn extends StatefulWidget {
  const CadNbn({Key? key}) : super(key: key);

  @override
  State<CadNbn> createState() => _CadNbnState();
}

class _CadNbnState extends State<CadNbn> {
  ...
  Future<void> loadSavedData() async {
    ...
    setState(() {
      _nomeController.text = prefs.getString('user2_nome') ?? '';
      _emailController.text = prefs.getString('user2_email') ?? '';
      _matriculaController.text = prefs.getString('user2_matricula') ?? '';
      _phoneController.text = prefs.getString('user2_telefone') ?? '';
      _equipeController.text = prefs.getString('user2_equipe') ?? '';
    });
  }

  Future<void> saveData() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('user2_nome', _nomeController.text);
    await prefs.setString('user2_email', _emailController.text);
    await prefs.setString('user2_matricula', _matriculaController.text);
    await prefs.setString('user2_telefone', _phoneController.text);
    await prefs.setString('user2_equipe', _equipeController.text);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Cadastro Narciso'),
      ),
